...
setlistener("/surface-positions/left-aileron-pos-norm", func (position){
	...
	var omm = getprop("/controls/LCR/old-mens-mode") or 0;
...
setlistener("/instrumentation/airspeed-indicator/indicated-speed-kt", func (speed){
	...
	if(getprop("/instrumentation/LCR/speed-indicator/selection")){
		if(groundspeed > 0.1){
			setprop("/instrumentation/LCR/speed-indicator/speed-meter", speed*1.15077945); # mph
		}else{
			setprop("/instrumentation/LCR/speed-indicator/speed-meter", 0);
		}
	}else{
		if(groundspeed > 0.1){
			setprop("/instrumentation/LCR/speed-indicator/speed-meter", speed*1.852); # km/h
		}else{
			setprop("/instrumentation/LCR/speed-indicator/speed-meter", 0);
		}
	}
...
setlistener("/instrumentation/LCR/speed-indicator/speed-limiter", func (sl){
...
setlistener("sim/model/start-idling", func()
 ...
 setlistener("/controls/gear/brake-parking", func(p)
  ...
  if (p.getValue() == 1)
   {
   		interpolate("/controls/LCR/driver-up", 1, 0.8);
   }
  else
   {
   		interpolate("/controls/LCR/driver-up", 0, 0.8);
   }
  }, 1, 0);
  ...
  if (!b.getBoolValue() and k == 109)
   {
		...
		var retrl = getprop("/LCR/race-lap") or 0;
		var rets = getprop("/LCR/this-sector") or 0;
		if(pa){
			var ra = {};
			var rn = 0;
			if(size(sectors) > 0){
				foreach(var s; sectors) {
					var as = getprop("/LCR/reset-store/"~pa~"/sector["~rn~"]/start-time") or 0;	
					ra[rn] = as;
			...
			if(size(sectors) > 0){
				settimer(func{
					foreach(var s; keys (ra)) {
						setprop("/LCR/"~pa~"/sector["~s~"]/start-time", ra[s]);		
					}
					find_marker();
				}, 10.0);
			}
		}
		setprop("/LCR/race-lap",retrl);
		setprop("/LCR/this-sector",rets);
