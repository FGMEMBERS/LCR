...
var racelap = props.globals.getNode("/LCR/race-lap");
var thissector = props.globals.getNode("/LCR/this-sector");
# only for mp result view
var sectortime = props.globals.getNode("/LCR/this-sector-time");
var laptime = props.globals.getNode("/LCR/this-lap-time");
var racetime = props.globals.getNode("/LCR/this-race-time");
var inrange = 0;
...
var total_result = func{
	var maxlaps = getprop("/LCR/"~pa~"/race-laps") or 0;
	var tr = 0;
    for(v = 0; v <= maxlaps; v +=1) {
		var actl = getprop("/LCR/"~pa~"/lap["~v~"]/actual-time") or 0;
		tr += actl;
	}
...
var show_lap_and_sector_time = func{
	var show_it = getprop("/LCR/show-race-times") or 0;
	var show_l_or_f = getprop("/LCR/switch-last-fastest") or 0;
	...
	if(show_it){
		...
		var mydriver = getprop("/sim/remote/pilot-callsign") or '';
		var who_is_the_driver = getprop("/dual-control/driver") or '';
		if(who_is_the_driver){
			mydriver = (mydriver == '') ? mydriver : "/"~mydriver;
			race_win.write(getprop("/sim/multiplay/callsign")~mydriver);
		}else{
			mydriver = (mydriver == '') ? mydriver : mydriver~"/";
			race_win.write(mydriver~getprop("/sim/multiplay/callsign"));
		}
	...
	foreach(var s; sectors) { 
		...
		var lsts = getprop("/LCR/"~pa~"/sector["~ln~"]/start-time") or 0;
		var sts  = getprop("/LCR/"~pa~"/sector["~n~"]/start-time") or 0;
		var lt  = getprop("/LCR/"~pa~"/sector["~n~"]/last-time") or 0;
		var ft  = getprop("/LCR/"~pa~"/sector["~n~"]/fastest-time") or 0;
	...
	var fl = getprop("/LCR/"~pa~"/fastest-lap") or 0;
	var ll = getprop("/LCR/last-lap-time") or 0;
...
var find_marker = func{
	...
	if(getprop("/LCR/"~pa~"/sector["~p~"]/fastest-time") == 0){
	...
	if(wp_distance < 260){
		var courserl = getprop("/LCR/"~pa~"/race-laps") or 0;
	...
	if(wp_distance < 50){
		setprop("/LCR/"~pa~"/sector["~thissector.getValue()~"]/start-time", getprop("/sim/time/elapsed-sec"));
		#print ("Sie ueberfuhren den "~thissector.getValue()~". Wegpunkt "~sectors[thissector.getValue()][3]);
		setprop("/LCR/sector",sectors[thissector.getValue()][3]);

		var fastesttime = getprop("/LCR/"~pa~"/sector["~ln~"]/fastest-time") or 0;
		var lastsectorstarttime = getprop("/LCR/"~pa~"/sector["~ln~"]/start-time") or 0;
		var lastsectorendtime = getprop("/LCR/"~pa~"/sector["~thissector.getValue()~"]/start-time") or 0;
		...
		setprop("/LCR/"~pa~"/sector["~ln~"]/last-time", lasttime);
		if(lasttime > 0 and lasttime < fastesttime or fastesttime == 0) setprop("/LCR/"~pa~"/sector["~ln~"]/fastest-time", lasttime);
		...
		if(thissector.getValue() == 0){
		  ...
		  foreach(var ls; sectors) { 
		  	totallapresult += getprop("/LCR/"~pa~"/sector["~n~"]/last-time") or 0;
			n+=1;
		  }
		  setprop("/LCR/last-lap-time", totallapresult);
		  var fastestlap = getprop("/LCR/"~pa~"/fastest-lap") or 0;
		  if(totallapresult > 0 and totallapresult < fastestlap or fastestlap == 0) setprop("/LCR/"~pa~"/fastest-lap", totallapresult);
		  setprop("/LCR/"~pa~"/lap["~racelap.getValue()~"]/actual-time", totallapresult);
...
var clear_race_datas = func{
	...
	if(pa != nil and inrange != nil and inrange == 1){
		...
		laptime.setValue(0);
		racetime.setValue(0);
		var act = props.globals.getNode("/LCR/"~pa);
		
		setprop("/LCR/last-lap-time",0);
