...
var virtual_racebikes_rec =  func{
	...
	if (wprec.getBoolValue() == 1){
		...
		settimer(virtual_racebikes_rec, 0.08);
	}		
}
...
var virtual_racebikes_play =  func{
		if(wpvr.getValue() >= 0 and wpplay.getBoolValue()) {
			...
			if(replaywp <= wpvr.getValue()){
			 if (foo != 0) foo.remove();
			 foo = geo.put_model("/Aircraft/LCR/Models/LCR-F2.xml", getprop("/virtual-racer/way/point["~replaywp~"]/lat"), getprop("/virtual-racer/way/point["~replaywp~"]/lon"), getprop("/virtual-racer/way/point["~replaywp~"]/elev-m") + 0.2, getprop("/virtual-racer/way/point["~replaywp~"]/hdg"), getprop("/virtual-racer/way/point["~replaywp~"]/pitch"));
			  #print(replaywp);
			  replaywp += 1;
			  if(replaywp > wpvr.getValue()) replaywp = 0;
			}
		}
		settimer(virtual_racebikes_play, 0.08);
}	
